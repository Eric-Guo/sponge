...
`, filepath.Join(dir, "test.sqlite3"), command)
			...
			cmd.Env = append(os.Environ(), "SPONGE_PROCESS_TEST_CONFIG="+cfgPath)
			if tt.parentSignal {
...
func TestServerProcessHelper(t *testing.T) {
	configFile := os.Getenv("SPONGE_PROCESS_TEST_CONFIG")
	if configFile == "" {
		return
	}
	flag.CommandLine = flag.NewFlagSet("serverNameExample", flag.ExitOnError)
