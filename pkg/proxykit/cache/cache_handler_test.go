...
func TestCacheBypassesRangeAndNoStore(t *testing.T) {
	for _, control := range []string{"public, max-age=60", "public, no-store, max-age=60", "private, public, max-age=60"} {
		t.Run(control, func(t *testing.T) {
			hits := 0
			handler := NewCacheHandler(newRecordingCache(), 1024, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				hits++
			...
			for i := 0; i < 2; i++ {
				handler.ServeHTTP(httptest.NewRecorder(), httptest.NewRequest("GET", "/asset", nil))
			}
			if control == "public, max-age=60" {
